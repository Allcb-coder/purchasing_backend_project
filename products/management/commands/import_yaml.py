...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "yaml_file", type=str, help="Path to the YAML file to import"
        )
        parser.add_argument(
            "--supplier",
            type=str,
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Importing from {yaml_file} for supplier {supplier_name}"
            )
        )
    ...
    def print_summary(self, stats):
        ...
        self.stdout.write(
            self.style.SUCCESS(f'Categories created: {stats["categories_created"]}')
        )
        self.stdout.write(
            self.style.SUCCESS(f'Products created: {stats["products_created"]}')
        )
        self.stdout.write(
            self.style.SUCCESS(f'Products updated: {stats["products_updated"]}')
        )
        self.stdout.write(
            self.style.SUCCESS(
                f'Supplier products created: {stats["supplier_products_created"]}'
            )
        )
        self.stdout.write(
            self.style.SUCCESS(
                f'Supplier products updated: {stats["supplier_products_updated"]}'
            )
        )
