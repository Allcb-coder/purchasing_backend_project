...
class ProductViewSet(viewsets.ModelViewSet):
    ...
    def featured(self, request):
        """Get featured products (example: first 10 products)"""
    ...
    def categories_summary(self, request):
        """Get summary of categories with product counts"""
...
# Add the missing import_yaml function
@api_view(['POST'])
@permission_classes([permissions.IsAdminUser])
def import_yaml(request):
    """Simple YAML import endpoint"""
    ...
    try:
        ...
        supplier_name = request.data.get('supplier', 'Imported Supplier')
                            ...
                            if name and category_id in categories:
                                product, created = Product.objects.get_or_create(
                                    ...
                                    defaults={
                                        ...
                                        'description': str(product_data.get('parameters', {}))
                                    }
                                )
                                if created:
                                    imported_count += 1
